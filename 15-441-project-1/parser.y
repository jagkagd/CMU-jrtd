...
// int yyerrok (Request* parsing_request, yyscan_t scanner, const char *s) {
...
%parse-param {Request *parsing_request}
...
t_token_char {
	YPRINTF("token: allowed_char_for_token: %c.\n", $1);
}; |
t_digit {
	$$ = '0' + $1;
}; |
...
request_line: token t_sp text t_sp t_version_header text t_crlf {
	YPRINTF("request_Line:\n%s\n%s\nHTTP/%s\n",$1, $3, $6);
	strcpy(parsing_request->http_method, $1);
	if (strcmp($3, "/") == 0) {
		strcpy(parsing_request->http_uri, "/index.html");
	} else {
...
request_headers: {
	YPRINTF("the last request_header\n");
}; |
